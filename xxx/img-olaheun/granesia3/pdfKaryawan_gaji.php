...
class PDF extends FPDF {
	...
	public function Header() {
		...
		//$this->Cell(80);
		//judul
		$this->Cell(0,20,'DAFTAR GAJI POKOK KARYAWAN DI PT. GRANESIA BANDUNG',0,0,'C');
		//pindah baris
		$this->Ln(20);
	...
	public function content() {
		...
		$this->Cell(25,5,"NIK",1,0,'C',true);
	    $this->Cell(40,5,"NAMA LENGKAP",1,0,'C',true);
		 $this->Cell(25,5,"NO REKENING",1,0,'C',true);
		
		$this->Cell(25,5,"JABATAN",1,0,'C',true);
		
		$this->Cell(25,5,"GOLONGAN",1,0,'C',true);
		
		
		$this->Cell(15,5,"SKR",1,0,'C',true);
		
		
		$this->Cell(30,5,"GAJI POKOK",1,0,'C',true);
		
		$this->Cell(30,5,"SISA GAJI",1,0,'C',true);
		
		$this->Cell(25,5,"PERIODE",1,0,'C',true);
  ...
  `gaji`.`gapok`
											...
											WHERE gaji.periode = '$bulan' AND gaji.hps = '0'";
										...
										if ($_GET['bulan'] != '0'){
										$bulan = $_GET['bulan'];
											$kondisi_bulan = " gaji.periode = '$bulan' ";
											$sql = $sql." AND ".$kondisi_bulan ;
										}
										
										if ($_GET['tahun'] != '0'){
										$tahun = $_GET['tahun'];
											$kondisi_tahun = " gaji.tahun = '$tahun' ";
											$sql = $sql." AND ".$kondisi_tahun ;
										}
		...
		if ($jmldata > 0){
				...
				while($hasil = mysql_fetch_array($result)){
					...
					// tampilkan cell
					$this->Cell(7,5,$no,1,0,'C',true);
					$this->Cell(25,5,$hasil["nik"],1,0,'R',true);
					$this->Cell(40,5,$hasil["nm_lengkap"],1,0,'L',true);
					 
					
		
					$this->Cell(25,5,$hasil["no_rek_payroll"],1,0,'R',true);
					$this->Cell(25,5,$hasil["nm_jabatan"],1,0,'L',true);
					$this->Cell(25,5,$hasil["nama_gol"],1,0,'L',true);
					
					
					$this->Cell(15,5,$hasil["jenis_gol"],1,0,'L',true);
					
					$this->Cell(30,5,number_format($hasil["gapok"],2),1,0,'R',true);
					$this->Cell(30,5,number_format($hasil["sisa_gaji"],2),1,0,'R',true);
					$this->Cell(15,5,bulan($hasil["periode"]),1,0,'L',true);
					$this->Cell(10,5,$hasil["tahun"],1,0,'L',true);
