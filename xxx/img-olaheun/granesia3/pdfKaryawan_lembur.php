...
class PDF extends FPDF {
	...
	public function Header() {
		...
		//$this->Cell(80);
		//judul
		$this->Cell(0,20,'DAFTAR LEMBURAN KARYAWAN DI PT. GRANESIA BANDUNG',0,0,'C');
		//pindah baris
		$this->Ln(20);
	...
	public function content() {
	    ...
	    $this->Cell(40,5,"NAMA LENGKAP",1,0,'C',true);
		 $this->Cell(25,5,"BAGIAN",1,0,'C',true);
		
		
		
		
		$this->Cell(25,5,"PEKERJAAN",1,0,'C',true);
		
		
		$this->Cell(35,5,"LOKASI",1,0,'C',true);
		$this->Cell(30,5,"KLASIFIKASI",1,0,'C',true);
 ...
 `field`.`nm_field`,
   ...
   Sum(`t_lem_karyawan`.`jumlah`) AS `jml`
											...
											WHERE lembur.periode = '$bulan' AND lembur.hps=0";
										...
										if ($_GET['bulan'] != '0'){
										$bulan = $_GET['bulan'];
											$kondisi_bulan = " lembur.periode = '$bulan' ";
		...
		if ($jmldata > 0){
				...
				while($hasil = mysql_fetch_array($result)){
					...
					$this->Cell(25,5,$hasil["nm_field"],1,0,'R',true);
					
					
					
					$this->Cell(25,5,$hasil["nm_pekerjaan"],1,0,'L',true);
					
					$this->Cell(35,5,$hasil["lokasi"],1,0,'R',true);
					$this->Cell(30,5,$hasil["klasifikasi"],1,0,'R',true);
					$this->Cell(30,5,number_format($hasil["jml"],2),1,0,'R',true);
