		...
		<?php include ('menuhistory.php');?>
				...
				<h3> History</h3>
					...
					<form method="post" class="form-search">
						<input type="text" name="modul" placeholder="Masukkan Pencarian Pada Modul"/>
						<a href='index.php?tab=datatabel&folder=history&file=history_form_view' class="btn btn-primary" ><i class='icon-list'></i>All</a>
					</form>
				...
				<table class="table table-condensed table-striped">
					...
					<?php
							...
							$query="SELECT * FROM history ORDER BY id_his DESC LIMIT $posisi,$batas";
							$query_page="SELECT * FROM history";

							//if(isset($_POST['modul'])){
								//$klasifikasi=$_POST['modul'];
							if(isset($_POST['modul'])||isset($_GET['modul'])){
							isset ($_POST['modul']) ? $modul = $_POST['modul'] : $modul = $_GET['modul'];
								
								$query="SELECT * FROM history WHERE modul LIKE '%$modul%' || user LIKE '%$modul%'
								|| data_awal LIKE '%$modul%' || data_akhir LIKE '%$modul%' || ket LIKE '%$modul%' LIMIT $posisi,$batas";
								$query_page="SELECT * FROM history WHERE modul LIKE '%$modul%' || user LIKE '%$modul%'
								|| data_awal LIKE '%$modul%' || data_akhir LIKE '%$modul%' || ket LIKE '%$modul%'";
							}
