  ...
  <?php
					...
					<table class="table table-condensed table-striped well">
                      ...
                      <tr class="info">
                        ...
                        <td> <?php
							...
							while($rows=mysql_fetch_array($result)){
							?>
							<a href="app/karyawan/pdf/<?php echo $rows['perjanjian_krj']?>"> <?php echo $rows['perjanjian_krj'];?> </a>
							
						
							<?php if(check_user("karyawan","upload_perjanjian_krj",$_SESSION['index'],$_SESSION['role_id']) == TRUE){?>
								<a href="index.php?tab=datatabel&folder=karyawan&file=perjanjian_krj_form_update&index=<?php  echo $rows['index']; ?>">
                      ...
                      <tr>
                        <td rowspan="2">Sertifikat Dan Personal File</td>
                        <td>&nbsp;<?php
							$query2="SELECT * FROM 
											 berkas_sertifikat
				...
				onclick="return confirm('Apakah anda yakin akan menghapus data ini?')" class="icon icon-trash pull-right"></a>
							<?php } ?>
											...
											data-toogle="tooltip" data-placement="bottom" title ="<?php echo $rows2['keterangan']; ?>">
											<?php
