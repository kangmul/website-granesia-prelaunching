...
<?php
	...
	if (isset($_SESSION['role_id'])){?>
		...
		<label class="badge badge-info"> Periode Bulan </label></br>
			...
			<a href='index.php?tab=datagaji&folder=gaji_karyawan&file=gaji_karyawan_form_view' class="btn btn-primary"><i class='icon-list'></i>All</a></br>
			</div>
			</tr>
			</table>
			</br>
			...
			<table width="750px">
				...
				<tr>
				  ...
				  <?php
					$sql_lokasi_kerja = "SELECT * FROM tkjp LEFT JOIN lokasi_kerja ON tkjp.id_lks_kerja = lokasi_kerja.id_lokasi where status = 'aktif' ";
				  ...
				  <td><select name='id_lks_kerja' id="lokasi_kerja" onchange="this.form.submit();">
					   ...
					   if($status_lokasi_kerja == 'ada' || $status_lokasi_kerja2 == 'ada' || 
							...
							while ($data=mysql_fetch_array($query)){
								if($id_lks_kerja == $data['id_lokasi'])
									echo "<option value=$data[id_lokasi] selected>$data[lokasi]</option>";
								else	
		...
		<?php
								...
								?>
									...
									<a href="pdfKaryawan_gaji.php?nama=<?php echo $nama; ?>
											...
											" target="_blank" class="btn-small btn-info btn">
...
?>
									...
									<a href="app/gaji_karyawan/modal_tambah_gaji.php" data-toggle="modal" data-target="#myModal" type="button" class="btn-small btn btn-success pilih">
  ...
  `gaji`.`gapok`
											...
											WHERE gaji.periode = '$bulan' AND gaji.tahun = '$tahun' AND hps=0
											LIMIT $posisi,$batas ";
											 
								$query_page="SELECT `gaji`.`id`,
  `gaji`.`tahun`,
  `gaji`.`periode`,
  `gaji`.`sisa_gaji`,
  `gaji`.`tgl_input`,
  ...
  `gaji`.`gapok`
						...
						<tr class="success">
							...
							<td><?php echo $rows['sisa_gaji']; ?></td>
							<td><?php echo $fungsi->fungsi->bulan($rows['periode']);?>&nbsp;<?php echo $rows['tahun'];?> </td>
							<td><?php echo $rows['tgl_input']; ?></td>
							<td align="center">
							<div class="btn-group">
								<a href="index.php?tab=datagaji&folder=gaji_karyawan&file=gaji_karyawan_form_insert&id=<?php echo $rows['id']; ?>" class="btn-mini btn btn-warning">
									<i class="icon icon-pencil"></i> Update</a>
									
								<?php if(check_user("karyawan","cetak_id_card",$_SESSION['index'],$_SESSION['role_id']) == TRUE){ ?>
									<a href="index.php?tab=datagaji&folder=gaji_karyawan&file=gaji_karyawan_act_delete&id=<?php echo $rows['id']; ?>" onclick="return confirm('Apakah anda yakin akan menghapus data ini?')" class="btn-mini btn btn-danger">
									<i class="icon icon-trash"></i> Delete</a>
								<?php 
								} 
								?>
								<a href="app/gaji_karyawan/gaji_karyawan_form_print.php?id=<?php echo $rows['id']; ?>" target="_blank" class="btn-mini btn btn-info">
