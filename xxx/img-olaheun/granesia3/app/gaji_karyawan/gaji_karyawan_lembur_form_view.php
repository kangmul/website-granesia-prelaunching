...
<?php
	...
	if (isset($_SESSION['role_id'])){?>
		...
		<?php
								...
								?>
									...
									<a href="app/gaji_karyawan/modal_tambah_lembur.php" data-toggle="modal" type="button" class="btn-small btn btn-success pilih" data-target="#myModal">
 ...
 `t_jabatan`.`nm_jabatan`,
  ...
  Sum(`t_lem_karyawan`.`jumlah`) AS `jml`
											...
											LEFT JOIN sistem_kerja ON tkjp.id_sistem_kerja = sistem_kerja.id_sistem_kerja
											WHERE lembur.periode = '$bulan' AND lembur.tahun = '$tahun' AND lembur.hps=0
											LIMIT $posisi,$batas ";
 ...
 `field`.`nm_field`,
  ...
  `lembur`.`id`";
						...
						<tr class="success">
							...
							<td align="center">
							<div class="btn-group">
								<a href="index.php?tab=datagaji&folder=gaji_karyawan&file=gaji_karyawan_lembur_form_insert&id=<?php echo $rows['id']; ?>" class="btn-mini btn btn-warning">
									<i class="icon icon-pencil"></i> Update</a>
									
								<?php if(check_user("karyawan","cetak_id_card",$_SESSION['index'],$_SESSION['role_id']) == TRUE){ ?>
									<a href="index.php?tab=datagaji&folder=gaji_karyawan&file=gaji_karyawan_lembur_act_delete&id=<?php echo $rows['id']; ?>" onclick="return confirm('Apakah anda yakin akan menghapus data ini?')" class="btn-mini btn btn-danger">
									<i class="icon icon-trash"></i> Delete</a>
								<?php 
								} 
								?>
								<a href="app/gaji_karyawan/gaji_karyawan_lembur_form_print.php?id=<?php echo $rows['id']; ?>" target="_blank" class="btn-mini btn btn-info">
